using Command.Contract.Abstractions;
using Command.Contract.Enumerations;

namespace Command.Contract.Errors
{
    /// <summary>
    ...
    public class Error(ErrorType type, params string[]? messages) : IError
    {
        ...
        public static IError TokenExpired(params string[]? messages)
        {
            return new Error(ErrorType.TokenExpired, messages);
        }
    }
}